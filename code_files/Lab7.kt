...
class Group{
    ...
    fun addStudents(student: Student){
        if (mapStudents.containsKey(student.getId())) {
            throw IllegalArgumentException("Student with index ${student.getId()} already exists in the group.")
        } else {
...
class GroupID<T> {
    ...
    fun getStudentByID(id: String): T {
        return mapStudents[id] ?: throw NoSuchElementException("Nie ma studenta o podanym numerze indeksu: $id")
    }
